...
class Example154 {
  public static void main(String[] args) { 
    FunList<Integer> empty = new FunList<>(null),
	  ...
	  list88 = list7.remove(7);						     		  //TEST AF 3.1.1	
	  //list88 = list7.filter((n)-> n > 12);	                  //TEST AF 3.1.3
	  //list88 = list7.removeFun(9, list7);
	...
	//System.out.println("Hallo: " + list7.get(2));
	
	System.out.println("her er min loesing");
	System.out.println("List 88" + list88);
...
class FunList<T> {
  ...
  protected static <T> Node<T> removeAll(T x, Node<T> xs) {
  ...
  public int count(Predicate<T> p) { 
	  ...
	  int count = 0;
	  return countAllP(p, xs, count);
  } 	

  protected static <T> Node<T> countAllP(Predicate<T> p, Node<T> xs, int count) {
    //check if node satisfies p  - if yes: count and move on to next... 
	return xs == null ? null : p.test(xs.item) ?  FunList.countAllP(p, xs.next, count++) : FunList.countAllP(p, xs.next, count);
  }
  ...
  */
  ...
  protected static <T> Node<T> filterAllOnPred(Predicate<T> p, Node<T> xs) {
	  return xs == null ? null : p.test(xs.item) ? new Node<T>(xs.item, filterAllOnPred(p, xs.next)) : filterAllOnPred(p, xs.next);
  }
  
  
  //Exercise 3.1.4 : WORK IN PROGRESS :) 
  public FunList<T> removeFun(T x) {
	  return new FunList<T>(this.stream().filter(xs -> xs.item != x).collect(Collectors.toList()));
  }
  
  //Exercise 3.1.5  
  // static method FunList<T> flatten(FunList<FunList<T>> xss) {}
  
  //Exercise 3.1.6  
  // flattenFun - using reduce, lambda expression and append...
  ...
  //Stream Builder
  public Stream buildMyStream(FunList myList) {
	Stream.Builder = new StringBuilder();

...
public class TestFetchWebGui {
  ...
  static class DownloadWorker extends SwingWorker<String,String> {
    ...
    public String doInBackground() {	
	  ...
	  //Setup task queue
	  //List<Callable> callList = new ArrayList<Callable>();
	  List<Future<?>> futures = new ArrayList<Future<?>>();
	  
	  for (int i=0; i<urls.length; i++) {
	  //System.out.println("Before submit to executor!!"); 	
	  final String j = urls[i]; 	
	  futures.add(executor.submit(() ->  {
		 return getPage(j, 200);
	  }));	
	...
	try {
      ...
      for (Future<?> fut : futures)
			if(fut.isDone()) {
				System.out.println("Future is back with res " +fut.get());
			}
	...
	return "Hello world";  
	  //make a runnable for each url
	  /*for (String url : urls) {
			final String my_url = url;
			Callable loadURLTask = new Callable() {	
				@Override
				public String call() {
					return getPage("www.itu.dk", 200);
				}	
			};	
			//executor.submit(loadURLTask);
			callList.add(loadURLTask);
	  }*/
	  
	  /*final Executor executor = Executors.newFixedThreadPool(urls.length);
		for (Callable c : callList) {
				executor.submit(c);
	  }
	  
	   try { return f.get(); } 
    catch (ExecutionException e) { System.out.println(e.getCause()); } */ 

	  /*int progress = 0;
		while (!taskQueue.isEmpty()) {
			Future loadTask = taskQueue.remove();
			if (loadTask.get()!= null) {
				progress += 100/urls.length;
				setProgress(progress);
				publish((String[])loadTask.get());
			}
		}*/
	}

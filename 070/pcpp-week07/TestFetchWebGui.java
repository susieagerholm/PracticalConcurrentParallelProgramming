...
public class TestFetchWebGui {
  public static void main(String[] args) {
	 //System.out.println("Der er ialt " + urls.length + " tasks..."); 
	 //int gaar_det_op = urls.length % 4;
	  //System.out.println("Modulo af tasks og workers er " + gaar_det_op);	
     //DownloadWorkPool dlp = new DownloadWorkPool(urls);
	 //dlp.runWorkersInPool();
	 badFetch();
    //goodFetch();
  }
  ...
  private static void goodFetch() {
	...
	DownloadWorkPool dwp = new DownloadWorkPool(urls, textArea);
    fetchButton.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {
          dwp.runWorkersInPool();
  ...
  static class DownloadWorkPool {
	  private DownloadWorker[] pool;
	  private final String[] work;
	  private int no_workers;
  
  
  public DownloadWorkPool(String[] work, TextArea text) {
	  work = work;
	  text = text;
	  no_workers = work.length;
	  pool = new DownloadWorker[no_workers];
  }
  
  public void createWorkersInPool() {
	  //Iterate through workers to give tasks 
	    for(int i = 0; i < work.length; i++) {
			  DownloadWorker downloadTask = new DownloadWorker(text, work[i]);
			  downloadTask.addPropertyChangeListener(new PropertyChangeListener() {
				public void propertyChange(PropertyChangeEvent e) {
			if ("progress".equals(e.getPropertyName())) {
            progressBar.setValue((Integer)e.getNewValue());
			}}});
			  //System.out.println( "Worker " + i + "faar task no." + j + " det er url: " + urls[j]);
		}
	}
  
  
  public void runWorkersInPool() {
    ...
    public String doInBackground() {
      ...
      for (String url : urls) {
        if (isCancelled())			    // (3)
          break;
        // System.out.println("Fetching " + url);
        String page = getPage(url2, 200),
          result = String.format("%-40s%7d%n", url2, page.length());
        // sb.append(result); // (1)
